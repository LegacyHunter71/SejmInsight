...
export async function getDeputies(
  ...
  if (
    res.number_of_elements !== undefined &&
    page.numberOfElements === undefined
  ) {
...
export async function getDeputyVotings(
  deputyId: number,
  params?: PageableParams & { vote?: string; title?: string },
) {
  const query: Record<string, any> = {
    ...(params
      ? { page: params.page, size: params.size, sort: params.sort }
  ...
  return api.get<
    PageDeputyListItemDto | components["schemas"]["PageVotingDto"]
  >(`/deputies/${deputyId}/votings`, { query });
}
...
export async function getDeputyById(deputyId: number) {
  ...
  const pageSize = 500;
  const res = await api.get<any>(`/deputies`, {
